...
//				tenSP = rs.getString("tenSP");
	...
	public boolean ThemSanPham(SanPham sanpham) {
		ConnectDB.getInstance();
		Connection con = ConnectDB.getConnection();
		String sql = "insert into SanPham (maSP,tenSP,gia,moTa,trangThai,hinhAnh,maLoaiSP,soLuong,maNCC) values(?,?,?,?,?,?,?,?,?) ";
		int k = 0;
		try {
			PreparedStatement preparedStatement = con.prepareStatement(sql);
	...
	public boolean SuaSanPham(SanPham sanpham) {
		ConnectDB.getInstance();
		Connection con = ConnectDB.getConnection();
		String sql = "update SanPham " + "set tenSP=?, gia=?"
				+ ",moTa=?, trangThai=?, hinhAnh=?, maLoaiSP=?, soLuong=?, maNCC=?" + " where maSP = ?";
		int k = 0;
		try {
			PreparedStatement preparedStatement = con.prepareStatement(sql);
			...
			preparedStatement.setBoolean(4, sanpham.getTrangThai());
			preparedStatement.setString(5, sanpham.getHinhAnh());
			preparedStatement.setString(6, sanpham.getLoaiSP().getMaLoaiSP());
			preparedStatement.setInt(7, sanpham.getSoLuong());
			preparedStatement.setString(8, sanpham.getMaNCC().getMaNCC());
			k = preparedStatement.executeUpdate();
	...
	public boolean updateSoLuong(int soLuong, String masp) {
		ConnectDB.getInstance();
		Connection con = ConnectDB.getConnection();
		...
		try {
			...
			k = preparedStatement.executeUpdate();
			System.out.println("Update unity success");
			return k>0;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	/*
	 * public Tour KiemTraCoTheChoHuongDanVien(String maTourMoi,String maHDV, Date
	 * ngayKH, Date ngayKT) { try { ConnectDB.getInstance(); Connection con =
	 * ConnectDB.getConnection(); String sql =
	 * "select * from Tour where maHuongDanVien = ?"; PreparedStatement
	 * preparedStatement = con.prepareStatement(sql); preparedStatement.setString(1,
	 * maHDV); ResultSet rs = preparedStatement.executeQuery(); while (rs.next()) {
	 * //Xét xem đây có bị trùng mã k, nếu có bỏ qua
	 * if(!maTourMoi.substring(3).equals(rs.getString(1).toString().substring(3))) {
	 * //Ngày khởi hành (ngày mới tạo) nằm trong khoảng ngày khởi hành và ngày kết
	 * thúc
	 * if(ngayKH.compareTo(rs.getDate(6))<=0&&ngayKH.compareTo(rs.getDate(5))>=0) {
	 * Tour tour = LayTourTheoMaTour(rs.getString(1)); return tour; } //Ngày khởi
	 * hành (mới tạo) và ngày kết thúc (mới tạo) bao cả ngày khởi hành và ngày kết
	 * thúc cũ
	 * if(ngayKH.compareTo(rs.getDate(5))<=0&&ngayKT.compareTo(rs.getDate(6))>=0) {
	 * Tour tour = LayTourTheoMaTour(rs.getString(1)); return tour; } //Ngày kết
	 * thúc (ngày mới tạo) nằm trong khoảng ngày khởi hành và ngày kết thúc cũ
	 * if(ngayKT.compareTo(rs.getDate(5))>=0&&ngayKT.compareTo(rs.getDate(6))<=0) {
	 * Tour tour = LayTourTheoMaTour(rs.getString(1)); return tour; } } } } catch
	 * (Exception e) { // TODO: handle exception } return null; }
	 */
	/*
	 * public Tour LayTourTheoMaTour(String mt) { try { ConnectDB.getInstance();
	 * Connection con = ConnectDB.getConnection(); String sql =
	 * "select * from Tour where maTour = ?"; PreparedStatement preparedStatement =
	 * con.prepareStatement(sql); preparedStatement.setString(1, mt); ResultSet rs =
	 * preparedStatement.executeQuery(); while (rs.next()) { String maTour =
	 * rs.getString(1); String tenTour = rs.getString(2); float giaTour =
	 * rs.getFloat(3); int soLuongNguoi = rs.getInt(4); Date ngayKhoiHanh =
	 * rs.getDate(5); Date ngayKetThuc = rs.getDate(6); String moTa =
	 * rs.getString(7); boolean tinhTrang = rs.getBoolean(8); String hinhAnh =
	 * rs.getString(9); HuongDanVien huongDanVien = new
	 * HuongDanVien(rs.getString(10)); DiaDanh diaDanh = new
	 * DiaDanh(rs.getString(11)); LoaiTour loaiTour = new
	 * LoaiTour(rs.getString(12)); Tour tour = new
	 * Tour(maTour,tenTour,giaTour,soLuongNguoi,ngayKhoiHanh,ngayKetThuc,moTa,
	 * tinhTrang,hinhAnh,huongDanVien,diaDanh, loaiTour); return tour; } } catch
	 * (Exception e) { // TODO: handle exception } return null; }
	 */
	...
	public ArrayList<SanPham> TimSP(String nhapSP, boolean loai) {
		// nếu loại là true thì tìm kiếm cho quản lý tour, false là tìm kiếm cho quản lý
		// vé
	...
	/*
	 * public void GuiEmail(SanPham SP, String NhaCC, String loaiSP) { try {
	 ...
	 * properties.put("mail.smtp.port", "587"); Session session =
	 * Session.getDefaultInstance(properties, new Authenticator() {
	 * 
	 * @Override protected PasswordAuthentication getPasswordAuthentication() { //
	 * TODO Auto-generated method stub return new
	 * PasswordAuthentication("lntvinh27122000@gmail.com","vinhhe2011"); }
	 * 
	 * }); Message message = new MimeMessage(session);
	 * message.setSubject("Một Tour du lịch vừa mới ra mắt !"); MimeBodyPart
	 * imgAttachment = new MimeBodyPart();
	 * imgAttachment.attachFile(tour.getHinhAnh()); MimeBodyPart tinNhan = new
	 * MimeBodyPart();
	 * 
	 * tinNhan.setContent("<h1>Thông báo một Tour du lịch sắp ra mắt !</h1><br>"
	 * ,"text/html; charset=UTF-8");
	 * 
	 * MimeBodyPart diaDiem = new MimeBodyPart(); //String dd=
	 * tour.getDiaDanh().getTenDiaDanh();
	 * diaDiem.setContent("<b>Địa điểm: </b>"+diaDanh+
	 * "<br>","text/html; charset=UTF-8"); MimeBodyPart loaiT = new MimeBodyPart();
	 * //String lt = tour.getLoaiTour().getTenLoaiTour();
	 * loaiT.setContent("<b>Loại tour: </b>"+loaiTour+
	 * "<br>","text/html; charset=UTF-8"); MimeBodyPart ngayKhoiHanh = new
	 * MimeBodyPart();
	 * ngayKhoiHanh.setContent("<b>Ngày khởi hành: </b>"+tour.getNgayKhoiHanh().
	 * toString()+"<br>","text/html; charset=UTF-8"); MimeBodyPart ngayKetThuc = new
	 * MimeBodyPart();
	 * ngayKetThuc.setContent("<b>Ngày kết thúc : </b>"+tour.getNgayKetThuc().
	 * toString()+"<br>","text/html; charset=UTF-8"); MimeBodyPart diaChi = new
	 * MimeBodyPart(); diaChi.setContent("<b>Địa chỉ: </b>"
	 * +" Số 12 Nguyễn Văn Bảo, Phường 4,Quận Gò Vấp, Thành phố Hồ Chí Minh"
	 * +"<br>","text/html; charset=UTF-8"); MimeBodyPart soDT = new MimeBodyPart();
	 * soDT.setContent("<b>Điện thoại: </b>"+"0327364753"+
	 * "<br>","text/html; charset=UTF-8"); MimeBodyPart email = new MimeBodyPart();
	 * email.setContent("<b>Email: </b>"+"lntvinh27122000@gmail.com"+
	 * "<br>","text/html; charset=UTF-8");
	 * 
	 * DecimalFormat formatter = new DecimalFormat("###,###,###"); MimeBodyPart
	 * giaVe = new MimeBodyPart();
	 * giaVe.setContent("<b>Giá vé: </b>"+formatter.format(tour.getGiaTour())+" VNĐ"
	 * +"<br>","text/html; charset=UTF-8");
	 * 
	 * 
	 * MimeMultipart multipart = new MimeMultipart();
	 * multipart.addBodyPart(tinNhan); multipart.addBodyPart(diaDiem);
	 * multipart.addBodyPart(loaiT); multipart.addBodyPart(ngayKhoiHanh);
	 * multipart.addBodyPart(ngayKetThuc); multipart.addBodyPart(giaVe);
	 * multipart.addBodyPart(diaChi); multipart.addBodyPart(soDT);
	 * multipart.addBodyPart(email); multipart.addBodyPart(imgAttachment);
	 * message.setContent(multipart); message.setFrom(new
	 * InternetAddress("lntvinh27122000@gmail.com"));
	 * message.setRecipient(RecipientType.TO, new
	 * InternetAddress("vinhmasxibua@gmail.com")); message.setSentDate(new
	 * java.util.Date());
	 * 
	 * Transport.send(message); } catch (Exception e2) {
	 * 
	 * } }
	 */
